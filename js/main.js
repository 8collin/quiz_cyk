...
Quiz.main = {
    ...
    start: async function () {
        ...
        // Перерисовка по любому событию. Стоит здесь, рядом с остальными
        // обработчиками, а не посреди загрузки: из загрузки есть ранний
        // выход (идущей игры нет), и стоявшее за ним присваивание не
        // выполнялось вовсе — обработчик оставался заглушкой. Ставить его
        // до входа в игру безопасно: зовут onChange только обработчики
        // подписки, а подписки до загрузки нет.
        ...
        document.addEventListener('visibilitychange', function () {
            ...
            Quiz.realtime.syncGame();
            // И то же самое для случая, когда сверять нечего, потому что
            // игры нет вовсе: пока вкладка была скрыта, её могли запустить.
            // Вне этого тупика вызов ничего не делает.
            Quiz.main.tryRecover();
        });
    ...
    /**
     * Как часто перепроверяем, не пора ли попробовать снова, когда игры не
     * нашлось или загрузка не удалась совсем. Тот же интервал, что у
     * сторожа realtime (WATCHDOG_MS), и по той же причине: это один
     * крохотный запрос, а не опрос базы.
     */
    RECOVERY_POLL_MS: 10000,

    /** Таймер этой перепроверки; заведён ровно тогда, когда играть не во что. */
    recoveryTimer: null,
        ...
        for (var attempt = 1; ; attempt++) {
            try {
                ...
                if (attempt >= this.LOAD_RETRIES) {
                    ...
                    // Дальше этот экран сам по себе не оживёт: подписки нет
                    // (заводится она в конце загрузки), а «Обновить» надо
                    // ещё нажать. Оставляем редкую перепроверку — она
                    // вернётся к загрузке, когда сеть поднимется.
                    this._startRecoveryPoll();
                    return;
                }
    ...
    loadGameOnce: async function () {
        ...
        if (!game) {
            ...
            // Подписки в этой ветке не будет: она заводится по id игры, а
            // id нет. То есть узнать, что ведущий игру запустил, клиенту
            // нечем — без перепроверки экран остался бы на «Игра не
            // найдена» до перезагрузки страницы.
            this._startRecoveryPoll();
            this.render();
            return;
        }

        // Игра нашлась — перепроверке больше нечего догонять.
    ...
    // --- Возврат из тупика «играть не во что» -----------------------------
    //
    // Пока идущей игры нет, у клиента нет и подписки: она заводится по id
    // игры. Значит, о том, что ведущий игру запустил, узнать нечем —
    // игрок, открывший страницу раньше ведущего, сидел на «Игра не
    // найдена» до F5. Так же кончал и тот, у кого загрузка не прошла все
    // LOAD_RETRIES раз: подписки нет, второго события не будет.
    //
    // Это та же болезнь, что в этапе 16 docs/decisions.md, — экран без
    // пути назад к верному состоянию, — и лечится тем же приёмом:
    // редкой сверкой раз в RECOVERY_POLL_MS, только пока вкладка на виду.
    // Игра нашлась — дальше обычная loadGame, тот же путь, которым клиент
    // входит в игру всегда.

    _startRecoveryPoll: function () {
        var self = this;
        if (this.recoveryTimer) return;
        this.recoveryTimer = setInterval(function () { self.tryRecover(); },
                                         this.RECOVERY_POLL_MS);
    },

    _stopRecoveryPoll: function () {
    ...
    /**
     * Один шаг сверки. Зовётся её таймером и из start(), когда вкладку
     * вернули на экран, — так же, как realtime.syncGame, и по той же
     * причине: пока экран был погашен, таймеры стояли, а ведущий нет.
     *
     * Сначала один крохотный запрос и только потом, если есть что
     * грузить, — полная loadGame: та снимает подписку, чистит store и
     * перечитывает звуки, и делать это раз в десять секунд впустую не за
     * чем. Вне тупика (таймер не заведён) не делает ничего.
     */
    tryRecover: async function () {
        if (!this.recoveryTimer) return;
        // Скрытую вкладку сверять незачем: посмотрят на неё не раньше, чем
        // сработает слушатель visibilitychange, а он сверится сам.
        if (document.hidden) return;
        // Загрузка уже идёт — не мешаем, её результат всё и решит.
        if (this.loading) return;
        // Подписка жива — значит, нас опередили («Обновить», ведущий,
        // переключивший игру), и догонять больше нечего.
        if (Quiz.realtime.wantConnection) {
            this._stopRecoveryPoll();
            return;
        }
        ...
        var game;
        try {
            game = await Quiz.db.getActiveGame();
        } catch (err) {
            // Сеть моргнула — следующая сверка попробует снова.
            return;
        }

        // База ответила, значит связь есть: полосу «переподключаюсь»,
        // если её показала сдавшаяся загрузка, пора убрать. Спорить тут не
        // с кем — сверка работает только там, где подписки нет.
        Quiz.ui.conn.setConnected(true);

        // Игры не было и нет — ничего не изменилось, уходим. Исключение —
        // store, оставшийся от неудавшейся загрузки: его надо привести в
        // порядок, иначе экран так и держит вопрос исчезнувшей игры.
        if (!game && !Quiz.store.game) return;

        try {
            await this.loadGame();
        } catch (err) {
            console.warn('Перепроверка: загрузка не удалась:', err.message);
        }
    },
    ...
    showLogin: function () {
        Quiz.realtime.unsubscribe();
        // Иначе перепроверка переживёт выход и продолжит ходить в базу без
        // сессии — раз в десять секунд, до перезагрузки страницы.
        this._stopRecoveryPoll();
