...
Quiz.game = {
    ...
    /**
     * Оценка, за которой сразу показывается ответ.
     *
     * Ровно score() плюс переключение show_answer — переключение, а не
     * «показать»: если ответ уже открыт, нажатие его закроет. На практике
     * это не мешает, потому что переход к вопросу сбрасывает show_answer,
     * и к моменту оценки ответ закрыт. С тех пор как отдельной кнопки
     * «Ответ» в шапке нет, это единственное место, где он открывается.
     *
     * Показ идёт последним. Он ни на что в подсчёте не влияет, а
     * освободить слот важнее: пока он занят, ось заморожена, и штраф
     * считается от того же T, на котором игрок нажал (см. score()).
     */
    scoreAndToggleAnswer: async function (participantId, delta) {
        await this.score(participantId, delta);
    ...
    /**
     * Удалить текущую игру со всем, что к ней привязано.
     *
     * Порядок не косметика. Сначала помечаем игру `finished` — это UPDATE,
     * и по нему подписки ИГРОКОВ узнают, что игра закрылась
     * (realtime.handleGame → onGameClosed уводит их с неё). Сам DELETE до
     * них не доходит: у канала game подписан только UPDATE, а не удаление
     * строки. Затем удаляем строку (дети уходят каскадом, см. db.deleteGame)
     * и перечитываем состояние — идущей игры может не остаться вовсе, и это
     * штатный «нет активной игры», из которого ведущий выберет или заведёт
     * другую.
     */
    deleteGame: async function () {
        ...
        await Quiz.db.setGameStatus(game.id, 'finished');
        await Quiz.db.deleteGame(game.id);
        await this.reload();
    },
