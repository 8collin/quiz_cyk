...
Quiz.db = {
    ...
    /**
     * Удаляет игру целиком. Всё, что к ней привязано, уходит каскадом:
     * вопросы, ответы, участники, буззер и журнал (внешние ключи объявлены
     * `on delete cascade`, см. 001_schema).
     */
    deleteGame: async function (gameId) {
        return this.unwrap(
            await this.client.from('game').delete().eq('id', gameId)
        );
    },
